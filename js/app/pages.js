...
function advancedPage() {
  return `<div class="page-grid advanced-page">${layerBar("Choose which layer to inspect or use for a key combination.")}${boardPanel({ full: true, title: "Advanced key map", description: "Every assigned feature receives its own color marker on the virtual keyboard." })}<section class="panel full-span advanced-feature-panel"><div class="advanced-feature-heading"><div><span class="eyebrow">ONBOARD BEHAVIOR LAB</span><h2>Advanced features</h2><p>Nine firmware behaviors, one consistent configuration workflow. Choose a card to configure it; use the information button for its captured tutorial.</p></div><div class="advanced-feature-count"><strong>09</strong><span>FEATURES</span></div></div><div class="advanced-grid advanced-dashboard">${ADVANCED_FEATURES.map((feature, index) => {
    const research = feature.code === "MACRO", status = research ? "POC · RESEARCH" : feature.ready ? "READY" : feature.placeholder ? "PLACEHOLDER" : "READ ONLY", mode = feature.mode === undefined ? feature.code === "COMBO" ? "LAYER MAP" : "FAMILY 07" : `MODE ${feature.mode}`;
    return `<article class="advanced-card feature-card ${feature.ready ? "ready" : ""} ${research ? "research" : ""}" style="--advanced-color:${feature.color}" ${feature.ready ? `data-advanced-config="${feature.code}"` : ""}><header><span class="advanced-card-index">${String(index + 1).padStart(2, "0")}</span><span class="advanced-card-status">${status}</span><button class="advanced-info-button" type="button" data-feature-info="${feature.code}" aria-label="About ${esc(feature.title)}" title="How ${esc(feature.title)} works">i</button></header><div class="advanced-card-symbol">${feature.code}</div><div class="advanced-card-copy"><h3>${esc(feature.title)}</h3><p>${esc(feature.body)}</p></div><footer><span>${mode}</span><b>${feature.ready ? "Configure" : "Inspect"}<i aria-hidden="true">→</i></b></footer></article>`;
