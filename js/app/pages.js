...
function spacebarLightingDots(key, live, light, paletteIndex, baseColor) {
  ...
  const custom = Boolean(light.customEnabled?.[key.id]), staged = state.dirty.customLighting.has(key.id);
  return `<span class="spacebar-lighting-leds" aria-hidden="true">${lightingMatrixIndexes(key).map((matrixIndex, led) => {
    const record = live && !staged ? state.hardware.liveMatrix[matrixIndex] : null, color = record ? rgbToHex(record) : custom ? light.perKey[key.id] : paletteIndex === 0 ? RAINBOW_PREVIEW[led % RAINBOW_PREVIEW.length] : baseColor;
      ...
      (row, uiRow) =>
        `<div class="keyboard-row">${row
          .map((_, col) => {
            ...
            const mapped = keycodeLabel(code), compactMapped = virtualKeycodeLabel(code, combinationTokenKnown(combinationBaseToken(layer, key.id)));
            const custom = Boolean(light.customEnabled?.[key.id]),
              stagedLighting = state.dirty.customLighting.has(key.id),
              liveIndex = lightingMatrixIndex(key),
              liveColor = live && !stagedLighting
...
function numberField(id, label, value, min, max, step, hint = "", disabled = false) {
  ...
  return `<label class="field ${disabled ? "disabled-field" : ""}"><span>${label}</span><div class="range-pair"><input type="range" min="${min}" max="${max}" step="${step}" value="${value}" data-range-for="${id}"${inactive}><input id="${id}" type="number" inputmode="decimal" min="${min}" max="${max}" step="${step}" value="${value}" data-keyboard-input="allow"${inactive}></div>${hint ? `<small>${hint}</small>` : ""}</label>`;
}
...
function keymapPage() {
  ...
  const layers = ["Main", "Fn1", "Fn2", "Fn3"];
  return `<div class="page-grid keymap-page">${layerBar("Choose a layer, then select a physical key to open its keymap.")}${boardPanel()}<section class="keymap-assignment-workspace"><div class="keymap-workspace-head"><div><span class="eyebrow">SELECTED KEY</span><h2>${esc(selectedKey().n)}</h2><p>${layers[Number(state.profile.layer)]} layer · ${esc(keycodeLabel(active))}</p></div><span class="keymap-workspace-code">0x${Number(active).toString(16).padStart(4, "0").toUpperCase()}</span></div>${selectedCard()}<div class="mapping-launcher"><div class="mapping-groups">${groups.map((group) => `<button type="button" data-mapping-group="${group}" class="${group === mappingGroup ? "active" : ""}">${group === "keyboard" ? "Keyboard" : group[0].toUpperCase() + group.slice(1)}</button>`).join("")}</div><p>Click any virtual key, or open the catalog directly. The picker keeps keyboard, media, mouse, firmware and lighting values separated.</p><button class="button primary" id="openMappingPicker" type="button">Open keymap catalog</button></div><div class="apply-row"><button class="button ghost" id="resetKeycode" type="button">Restore physical default</button></div></section></div>`;
}
...
function lightingPalettePanel(base, palette, target) {
  const paletteIndex = Math.max(0, Math.min(7, Number(base.paletteIndex))),
    ...
    rgbPrefix = target === "main" ? "paletteRgb" : "stripPaletteRgb",
    presets = [
      "#ff3b30",
      "#ff9500",
      "#ffcc00",
...
function advancedAssignmentsPanel() {
  ...
  return `<section class="panel full-span advanced-assignments"><div class="panel-head"><div><h2>Assigned advanced features</h2><p>Detected onboard assignments are grouped by action. Removals stay staged until Apply changes.</p></div><span class="badge ${entries.length ? "ready" : ""}">${entries.length} ASSIGNED</span></div><div class="advanced-assignment-list">${entries.length ? entries.map((entry) => {
    const removeAttribute = entry.macro
      ? `data-remove-macro="${entry.layer}:${entry.ids[0]}"`
      : entry.combination
...
function advancedPage() {
  return `<div class="page-grid advanced-page">${layerBar("Choose which layer to inspect or use for a key combination.")}${boardPanel({ full: true, title: "Advanced key map", description: "Every assigned feature receives its own color marker on the virtual keyboard." })}<section class="panel full-span advanced-feature-panel"><div class="advanced-feature-heading"><div><span class="eyebrow">ONBOARD BEHAVIOR LAB</span><h2>Advanced features</h2><p>Nine firmware behaviors, one consistent configuration workflow. Choose a card to configure it; use the information button for its captured tutorial.</p></div><div class="advanced-feature-count"><strong>09</strong><span>FEATURES</span></div></div><div class="advanced-grid advanced-dashboard">${ADVANCED_FEATURES.map((feature, index) => {
    const status = feature.ready ? "READY" : feature.placeholder ? "PLACEHOLDER" : "READ ONLY", mode = feature.mode === undefined ? feature.code === "COMBO" ? "LAYER MAP" : "FAMILY 07" : `MODE ${feature.mode}`;
    return `<article class="advanced-card feature-card ${feature.ready ? "ready" : ""}" style="--advanced-color:${feature.color}" ${feature.ready ? `data-advanced-config="${feature.code}"` : ""}><header><span class="advanced-card-index">${String(index + 1).padStart(2, "0")}</span><span class="advanced-card-status">${status}</span><button class="advanced-info-button" type="button" data-feature-info="${feature.code}" aria-label="About ${esc(feature.title)}" title="How ${esc(feature.title)} works">i</button></header><div class="advanced-card-symbol">${feature.code}</div><div class="advanced-card-copy"><h3>${esc(feature.title)}</h3><p>${esc(feature.body)}</p></div><footer><span>${mode}</span><b>${feature.ready ? "Configure" : "Inspect"}<i aria-hidden="true">→</i></b></footer></article>`;
